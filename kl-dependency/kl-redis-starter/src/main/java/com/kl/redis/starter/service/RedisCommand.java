...
import com.kl.redis.starter.properties.StrPool;
...
public class RedisCommand {
    ...
    private static final String CACHE_KEY_NOT_NULL = "String不能为空";
    ...
    public RedisCommand(RedisTemplate<String, Object> redisTemplate, StringRedisTemplate stringRedisTemplate, boolean defaultCacheNullVal) {
    ...
    /**
     * 将对象手动转为JSON字符串值 value 存放到 key 。序列化和反序列化可以不是同一个class 适合简单对象
     * <p>
     ...
     */
    public void setToJsonStr(@NonNull String key, Object value) {
     ...
     * @see <a href="https://redis.io/commands/set">Redis Documentation: SET</a>
     */
    public void setToJsonStr(@NonNull String key, Object value, long timeout, TimeUnit unit) {
        String json = JsonUtils.objectToJson(value);
        stringRedisTemplate.opsForValue().set(key, json);
    }

    /**
     * 返回与键 key 相关联的 value JSON值 并手动转为对象 序列化和反序列化可以不是同一个class 适合简单对象
     * <p>
     ...
     */
    @Nullable
    public <T> T getFromJsonStr(@NonNull String key, Class claz) {
        String json = stringRedisTemplate.opsForValue().get(key);
        if (StringUtils.isEmpty(json)) {
            return null;
        }
        return (T) JsonUtils.jsonToObject(json, claz);
    }

    /**
     * 将字符串值 value 存放到 key 。 序列化和反序列化必须是同一个class 适合复杂对象
     * <p>
     * 如果 key 已经持有其他值， SET 就覆写旧值， 无视类型。
     * 当 SET 命令对一个带有生存时间（TTL）的键进行设置之后， 该键原有的 TTL 将被清除.
     *
     ...
     * @see <a href="https://redis.io/commands/set">Redis Documentation: SET</a>
     */
    public void set(@NonNull String key, Object value, long timeout, TimeUnit unit, boolean... cacheNullValues) {
        boolean cacheNullVal = cacheNullValues.length > 0 ? cacheNullValues[0] : defaultCacheNullVal;
        if (!cacheNullVal && value == null) {
            return;
        }

        valueOps.set(key, value == null ? newNullVal() : value, timeout, unit);
    }
     ...
     * @see <a href="https://redis.io/commands/setex">Redis Documentation: SETEX</a>
     */
    public void setEx(@NonNull String key, Object value, int expireTime, TimeUnit timeUnit) {
        setEx(key, value, timeUnit.toSeconds(expireTime));
    }
    ...
    /**
     * 返回与键 key 相关联的 value 值 序列化和反序列化必须是同一个class 适合复杂对象
     * <p>
    ...
    private Map<String, Object> mSetMap(@NonNull Map<String, Object> map, boolean cacheNullVal) {
        Map<String, Object> mSetMap = new HashMap<>(16);
        map.forEach((k, v) -> {
    ...
    public <K, V> void hmSet(@NonNull String key, @NonNull Map<K, V> hash, boolean... cacheNullValues) {
        boolean cacheNullVal = cacheNullValues.length > 0 ? cacheNullValues[0] : defaultCacheNullVal;
        Map<Object, Object> newMap = new HashMap<>(16);
     ...
     */
    public <V> Long sAdd(@NonNull String key, V... members) {
        Long count = setOps.add(key, members);
        return count;
    }
     ...
     * @see <a href="https://redis.io/commands/sadd">Redis Documentation: SADD</a>
     */
    public <V> Long sAdd(@NonNull String key, long timeout, TimeUnit unit, Collection<V> members) {
        Long count = setOps.add(key, members.toArray());
     ...
     */
    @Nullable
    public <V> Set<V> sRandMember(@NonNull String key, long count) {
     ...
     */
    @Nullable
    public <V> List<V> sRandMembers(@NonNull String key, long count) {
     ...
     */
    public <V> Boolean sMove(@NonNull String sourceKey, String destinationKey, V value) {
     ...
     */
    @Nullable
    public <V> Set<V> sMembers(@NonNull String key) {
     ...
     */
    @Nullable
    public <V> Set<V> sInter(@NonNull String key, @NonNull String otherKey) {
    ...
    public Set<Object> sInter(@NonNull String key, Collection<String> otherKeys) {
        return setOps.intersect(key, otherKeys.stream().map(String::toString).collect(Collectors.toList()));
    }
     ...
     */
    @Nullable
    public <V> Set<V> sInter(Collection<String> otherKeys) {
        return (Set<V>) setOps.intersect(otherKeys.stream().map(String::toString).collect(Collectors.toList()));
    }
     ...
     */
    @Nullable
    public Long sInterStore(@NonNull String key, @NonNull Collection<String> otherKeys, @NonNull String destKey) {
        return setOps.intersectAndStore(key,
                otherKeys.stream().map(String::toString).collect(Collectors.toList()), destKey);
    }
    ...
    public Long sInterStore(Collection<String> otherKeys, @NonNull String destKey) {
        return setOps.intersectAndStore(otherKeys.stream().map(String::toString).collect(Collectors.toList()), destKey);
    }
     ...
     */
    @Nullable
    public <V> Set<V> sUnion(@NonNull String key, @NonNull String otherKey) {
     ...
     */
    @Nullable
    public <V> Set<V> sUnion(@NonNull String key, Collection<String> otherKeys) {
        return (Set<V>) setOps.union(key, otherKeys.stream().map(String::toString).collect(Collectors.toList()));
    }
    ...
    public <V> Set<V> sUnion(Collection<String> otherKeys) {
        return (Set<V>) setOps.union(otherKeys.stream().map(String::toString).collect(Collectors.toList()));
    }
    ...
    public Long sUnionStore(Collection<String> otherKeys, @NonNull String distKey) {
        return setOps.unionAndStore(otherKeys.stream().map(String::toString).collect(Collectors.toList()), distKey);
    }
     ...
     */
    @Nullable
    public <V> Set<V> sDiff(@NonNull String key, @NonNull String otherKey) {
    ...
    public <V> Set<V> sDiff(Collection<String> otherKeys) {
        return (Set<V>) setOps.difference(otherKeys.stream().map(String::toString).collect(Collectors.toList()));
    }
    ...
    public Long sDiffStore(Collection<String> otherKeys, @NonNull String distKey) {
        return setOps.differenceAndStore(otherKeys.stream().map(String::toString).collect(Collectors.toList()),
                distKey);
    }
